...
ActFF = 1;         % Enable Feedback Forward Decoupling

Ts   = 1e-6;       % Fundamental sample time            [s]
fsw  = 10e3;       % PMSM drive switching frequency fpwm = 10kHz Tpwm = 1/fsw = 100us     [Hz]
Tsi  = 5e-5;       % Sample time for inner control loop [s]
Tso  = 50e-5;      % Sample time for outer control loop [s]
Tpwm = 1/fsw;

sigma = 80e-6;    % 等效电流环路延时

% 典型Ⅱ型系统整定方法 
% Kp = 0.6*Ls/sigma;  Ki = Kp/5sigma = 0.6*Ls/(5*sigma*sigma);
...
Kp_omega = 0.2;     % 10  % Proportional gain velocity controller
Ki_omega = 130;    % 200 % Integrator gain velocity controller

% fpwm = fsw = 10kHz, f_n = (n*2*pi/60)*p/(2*pi) = n*(2/15), we = (n*2*pi/60)*p
% 载波比 = fpwm/f_n
